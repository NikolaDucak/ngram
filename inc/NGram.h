...
class NGram {
    ...
    std::unordered_map<Gram_t, WeightedContinuations_t, GramHasher<type, n>>
        map;
    std::set<type> items;
...
public:
    explicit NGram(std::vector<type> sequence) {
        ...
        for (auto i = sequence.begin(); i != sequence.end() - n; i++) {
            map[createGram(i)].countContinuation(
                get_refrence_to_gram_in_map(createGram(i + 1)));
        }
    }

    const std::set<type>& getItems() const { return items; }
    ...
    std::vector<NGramState_t> getState(LookUpGram<type> search) const {
        if (search.n > n)
            throw std::out_of_range(
                "lookup gram has too many items");    
...
private:
    ...
    const Gram_t& get_refrence_to_gram_in_map(Gram_t g) {
        auto insertion =
            map.insert(std::make_pair(g, WeightedContinuations_t()));
